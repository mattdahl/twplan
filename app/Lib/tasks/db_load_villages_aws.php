<?php
/**
 * Intended for cron job usage for live development. Loads village data into the RDS database.
 * Crontab example:
 * /usr/local/bin/php ~/beta/app/Lib/tasks/db_load_villages_aws.php >/dev/null 2>&1
 */
...
try {
	log_success("** BEGIN VILLAGES LOAD **\n");
	...
	$world_query = "SELECT `world_number` FROM `worlds` ORDER BY `villages_last_updated` LIMIT 1";
	...
	$local_filepath = TWPLAN_CONFIG::$app_path . '/app/tmp/data/villages/' . $server_prefix . $world . '_village_data.txt';
	...
	if (!$village_file) {
		...
		log_success("Loaded remote village data... \n");
		$write_to = '';
	...
	while (($data = fgetcsv($local_filepath_handle, 1000, ">"))) {
		...
		$mysqli->query($load_query) or log_error(sprintf("Error loading village data with query \n %s \n Error message: %s \n", $load_query, $mysqli->error));
	}
	...
	$update_time_query = "UPDATE `worlds` SET villages_last_updated='$current_time' WHERE `world_number` = '$world'";
	...
	mail("site@twplan.com", "db_load_villages_aws ERROR (W" . $world . ")", $e->getMessage());
	exit();
}

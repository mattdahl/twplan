...
       
	          if ((late-early) < 0) { // allows the restriction to "wrap around" the night
	              if (diff >= early || diff < late) { // "good" match
		             villages[i].candidates.unshift(targets[j]);
					 villages[i].goodMatches++;
					 
					 targets[j].candidates.unshift(villages[i]);
					 targets[j].goodMatches++;
	  			  }
	              else { // "bad" match
...
function doMarriage(v, t, s) {
	...
	function sortByLeastGoodMatches(a, b) { // the coordinates with the least good matches should be first
		return b.goodMatches - a.goodMatches;
	}
	
	villages.sort(sortByLeastGoodMatches);
	targets.sort(sortByLeastGoodMatches);
